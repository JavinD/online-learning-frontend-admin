...
export default function CourseEditPage() {
  ...
  const { course } = useSelector((state: RootState) => state.courseDetail);
  ...
  const [category, setCategory] = useState<ICategory>(
    course?.category ? course.category : { id: 0, name: "" }
  );
  const [currentTags, setCurrentTags] =
    useState<{ value: string; label: string }[]>();
  const [price, setPrice] = useState<number>(course?.price ? course.price : 0);
  const [summaryDesc, setSummaryDesc] = useState<string>(
    course?.summary_desc ? course.summary_desc : ""
  );
  const [content, setContent] = useState<string>(
    course?.content ? course.content : ""
  );
  const [author, setAuthor] = useState<string>(
    course?.author_name ? course.author_name : ""
  );
  ...
  useEffect(() => {
    if (course) {
      ...
      setCurrentTags(
        course.tag.map((tag) => ({ value: tag.id.toString(), label: tag.name }))
      );
      setPrice(course.price);
      setSummaryDesc(course.summary_desc);
      setContent(course.content);
      setAuthor(course.author_name);
      setImgPreview(course.img_url);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    fetch(`${API_URL}/course/${course?.slug}`, {
      method: "PUT",
      headers: {
      ...
      .catch((error) => {
        setCourseLoading(false);
        toastFailed("Failed to save data");
      });
  };
  ...
  }, [cookies, courseDetailDispatch, slug]);
